...
import type { GridChartData } from '../api'
...
const props = defineProps<{ data: GridChartData | null }>()
...
function buildOption(d: GridChartData): echarts.EChartsOption {
  ...
  const series: echarts.SeriesOption[] = [
    ...
    },
    {
      ...
      // 网格水平线：每条标注价格（虚线，不抢色彩焦点）
      markLine: {
        symbol: 'none',
        silent: true,
        lineStyle: { type: 'dashed', width: 1, color: tc.axisLine },
        label: { color: tc.axisLabel, fontSize: 10, position: 'insideEndTop' },
        data: d.grid_levels.map((p) => ({ yAxis: p, label: { formatter: p.toFixed(2) } })),
      },
    },
  ]
  const legendData = ['市值', '成本', '亏损', '盈利', '收益率', '收盘价']
  if (hasBench) {
  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        const amtLabel = sig === 'buy' ? '买入金额' : sig === 'sell' ? '卖出回款' : ''
        return (
          `${d.dates[i]}` +
          (sig !== 'hold'
            ? `　<span style="color:${sigColor};font-weight:600">📌 ${SIGNAL_LABEL[sig]}</span>`
            : '') +
          (amt != null
            ...
            : '') +
          `<br/>价格：${f2(close)}　网格：<span style="color:${tc.axisLabel}">第 ${gi} 格</span>` +
          `<br/>持仓：${f2(hold)}` +
